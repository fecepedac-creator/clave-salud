...
export async function goToAdminDashboard(page: Page) {
    ...
    await page.goto(
        `${TEST.BASE_URL}/center/${TEST.CENTER_ID}?agent_test=true`
    );
...
export async function goToAdminPerformanceTab(page: Page) {
    ...
    await expect(
        page.locator('[data-testid="performance-loading-skeleton"]')
...
export async function goToDoctorDashboard(page: Page) {
    await disableAnimations(page);
    await page.goto(
        `${TEST.BASE_URL}/pro/center/${TEST.CENTER_ID}?agent_test=true`
    );
