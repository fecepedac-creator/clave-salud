...
test.describe("E2E Navigation & UX", () => {
    ...
    test("E2E-01: Login Admin + dashboard + Close Panel", async ({ page }) => {
        ...
        await expect(page.locator('[data-testid="view-container-home"], [data-testid="view-container-select-center"]')).toBeVisible();
    });
    ...
    test("E2E: Flujo Navegación Paciente (Breadcrumbs)", async ({ page }) => {
        ...
        // 2. Entrar a reserva
        const bookingBtn = page.locator('button:has-text("Reserva de Horas"), [data-testid="btn-patient-booking"]');
    ...
    test("E2E-11: Reserva pública exitosa", async ({ page }) => {
        ...
        const hourBtn = page.locator('button:has-text("09:00"), button:has-text("10:00"), button:has-text("11:00")').first();
        ...
        // 8. Verifica éxito
        await expect(page.locator('text=/¡Reserva confirmada!|¡Cita agendada!/')).toBeVisible({ timeout: 15000 });
    });
