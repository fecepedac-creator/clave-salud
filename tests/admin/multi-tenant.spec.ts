...
test("T9 — Seguridad Multi-Tenant: Admin no ve KPIs de otro centro", async ({
    page,
}) => {
    // 1. Intentar acceder directamente al Centro B con sesión del Admin del Centro A
    await page.goto(
        `${TEST.BASE_URL}/center/${CENTRO_B_ID}?agent_test=true`
    );
    ...
    if (!tabBarVisible) {
        ...
        console.log(
            "✅ T9 — Guard activo: admin redirigido fuera del Centro B."
        );
        return;
    }
    ...
    if (await perfTab.isVisible()) {
        ...
        if (await statsTable.isVisible()) {
            ...
            const hasRealData =
                /\$[\d.]+|350\.000|Doctor Test/.test(tableText) &&
                !tableText.includes("Sin datos");
            ...
            console.log(
                "✅ T9 — Multi-tenant OK: tab de rendimiento no muestra datos del Centro B."
            );
        }
    } else {
        // Si el tab de rendimiento no carga, el guard está funcionando
        console.log(
            "✅ T9 — Tab rendimiento no accesible para Centro B. Guard activo."
        );
    }
    ...
    if (await kpiEl.isVisible()) {
        ...
        console.log(
            `✅ T9 — KPI total del Centro B = ${kpiValue} (correcto: sin datos seeded).`
        );
    }
});
