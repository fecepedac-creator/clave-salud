...
test.describe("Fase 1: Concurrencia de Agendamiento", () => {
  test("Simular reserva simultánea - Solo una debe tener éxito", async ({ browser }) => {
    ...
    const prepararReserva = async (page, name, rut) => {
      ...
      const centerCard = page.locator('div').filter({ hasText: /^Centro Medico Los Andes/i }).first();
      ...
      const careTypeBtn = page.locator('div').filter({ hasText: /^Consulta Médica/ }).first();
      ...
      const targetDay = page.locator('.bg-white.text-emerald-700').filter({ hasText: /^23$/ }).first();
      
      if (!(await targetDay.isVisible())) {
          console.log(`[${name}] Día 23 no detectado como disponible directamente, buscando cualquier slot verde...`);
    ...
    if (totalExitos > 1) {
        throw new Error("CRÍTICO: ¡Se ha permitido OVERBOOKING! Ambos pacientes reservaron el mismo slot.");
    }
