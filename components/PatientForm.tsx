...
import { validateRUT, formatRUT, generateId, capitalizeWords, sanitizeText, formatChileanPhone, extractChileanPhoneDigits, formatPersonName } from '../utils';
...
const BigInput = ({ label, value, onChange, placeholder, type = "text", error }: any) => (
  <div className="mb-4">
      ...
      />
      {error && <p className="text-red-600 font-bold mt-1 text-sm flex items-center gap-1"><AlertCircle className="w-4 h-4"/> {error}</p>}
  </div>
);
...
const SelectionCard = ({ label, selected, onClick }: any) => (
    ...
    >
        <span className={`text-lg md:text-xl font-bold ${selected ? 'text-blue-800' : 'text-slate-600 group-hover:text-slate-800'}`}>{label}</span>
        <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center transition-colors ${selected ? 'bg-blue-600 border-blue-600' : 'border-slate-300'}`}>
...
const PatientForm: React.FC<PatientFormProps> = ({
  ...
  const handleSave = () => {
    ...
    const finalDrugDetails = drugUse === 'Si' && drugDetails ? capitalizeWords(sanitizeText(drugDetails)) : undefined;
    const finalMedicalDetails = otherMedicalDetails ? capitalizeWords(sanitizeText(otherMedicalDetails)) : undefined;
    const finalCancerDetails = cancerDetails ? capitalizeWords(sanitizeText(cancerDetails)) : undefined;
    const finalSurgicalDetails = otherSurgicalDetails ? capitalizeWords(sanitizeText(otherSurgicalDetails)) : undefined;
    const finalHerniaDetails = herniaDetails ? capitalizeWords(sanitizeText(herniaDetails)) : undefined;
      ...
      <div className="bg-white sm:rounded-b-3xl shadow-xl border border-slate-100 overflow-hidden">
        <div className="p-6 md:p-10 space-y-10">
            ...
            <div className="animate-fadeIn">
                ...
                <p className="text-slate-500 text-lg mb-8">Por favor, escriba sus datos tal como aparecen en su carnet.</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="md:col-span-2">
                        <BigInput label="Nombre Completo" value={fullName} onChange={(e: any) => setFullName(e.target.value)} error={errors.fullName} />
                    </div>
                    ...
                    <div className="md:col-span-2 border-t border-slate-100 pt-6 mt-2">
                        <BigInput label="Ocupación / Oficio" value={occupation} onChange={(e: any) => setOccupation(e.target.value)} placeholder="Ej: Profesor, Dueña de casa..." />
                    </div>

                    <div className="md:col-span-2">
                         <label className="block text-xl font-bold text-slate-700 mb-4 flex items-center gap-2">
                            <Users className="w-5 h-5"/> ¿Con quién vive? <span className="text-sm font-normal text-slate-400">(Puede marcar varios)</span>
                         </label>
                         {errors.livingWith && <p className="text-red-600 font-bold mb-2 text-sm">{errors.livingWith}</p>}
            ...
            <div className="animate-fadeIn">
                ...
                <p className="text-slate-500 text-lg mb-8">Seleccione las opciones tocando los recuadros correspondientes.</p>
                
                {/* Patologías */}
                <div className="mb-10">
                    <h4 className="text-xl font-bold text-slate-800 mb-4 bg-slate-100 p-3 rounded-lg inline-block">¿Tiene alguna de estas enfermedades?</h4>
                    ...
                    {medicalHistory.includes('CANCER') && (
                        <div className="mt-4 p-6 bg-red-50 rounded-2xl border-2 border-red-100">
                             <label className="block text-xl font-bold text-red-800 mb-2">¿Qué tipo de cáncer?</label>
                             <input value={cancerDetails} onChange={(e) => setCancerDetails(e.target.value)} className="w-full p-4 text-lg border-2 border-red-200 rounded-xl focus:border-red-400 outline-none" placeholder="Ej: Mama, Próstata..." />
                        </div>
                    )}
                    {medicalHistory.includes('OTRO') && (
                        <div className="mt-4 p-6 bg-slate-50 rounded-2xl border-2 border-slate-200">
                             <label className="block text-xl font-bold text-slate-700 mb-2">¿Qué otra enfermedad?</label>
                             <textarea value={otherMedicalDetails} onChange={(e) => setOtherMedicalDetails(e.target.value)} className="w-full p-4 text-lg border-2 border-slate-300 rounded-xl" rows={2} />
                        </div>
                    )}
                </div>
                ...
                {/* Cirugías */}
                <div className="mb-10">
                     <h4 className="text-xl font-bold text-slate-800 mb-4 bg-slate-100 p-3 rounded-lg inline-block">¿Le han realizado cirugías?</h4>
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                     ...
                     {surgicalHistory.includes('HERNIA') && (
                        <div className="mt-4 p-6 bg-indigo-50 rounded-2xl border-2 border-indigo-100">
                             <label className="block text-xl font-bold text-indigo-900 mb-2">¿Dónde fue la hernia?</label>
                             <input value={herniaDetails} onChange={(e) => setHerniaDetails(e.target.value)} className="w-full p-4 text-lg border-2 border-indigo-200 rounded-xl" placeholder="Ej: Inguinal..." />
                        </div>
                    )}
                </div>
                    ...
                    <div className="bg-slate-50 p-6 rounded-3xl border-2 border-slate-100">
                        ...
                        <div className="flex flex-col gap-3">
                            {['No fumador', 'Ex fumador', 'Fumador actual'].map(opt => (
                                <button key={opt} onClick={() => { setSmoking(opt); if(opt === 'No fumador') { setCigsPerDay(''); setYearsSmoking(''); } }} className={`p-4 rounded-xl text-lg font-bold border-2 text-left transition-all ${smoking === opt ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-500 border-slate-200'}`}>
                                    {opt}
                                </button>
                            ))}
                        </div>
                        {smoking !== 'No fumador' && (
                             <div className="mt-4 space-y-4">
                                 <div><label className="font-bold text-slate-600">Cigarros al día</label><input type="number" value={cigsPerDay} onChange={e => setCigsPerDay(e.target.value)} className="w-full p-3 border-2 rounded-xl text-lg"/></div>
                                 <div><label className="font-bold text-slate-600">Años fumando</label><input type="number" value={yearsSmoking} onChange={e => setYearsSmoking(e.target.value)} className="w-full p-3 border-2 rounded-xl text-lg"/></div>
                             </div>
                        )}
                    </div>
                    ...
                    <div className="bg-orange-50 p-6 rounded-3xl border-2 border-orange-100">
                        ...
                        <div className="flex flex-col gap-3">
                            {['No consumo', 'Ocasional', 'Frecuente'].map(opt => (
                                <button key={opt} onClick={() => setAlcohol(opt)} className={`p-4 rounded-xl text-lg font-bold border-2 text-left transition-all ${alcohol === opt ? 'bg-orange-500 text-white border-orange-500' : 'bg-white text-slate-500 border-orange-200'}`}>
                                    {opt}
                                </button>
                            ))}
                        </div>
                ...
                <div className="mt-8 bg-purple-50 p-6 rounded-3xl border-2 border-purple-100">
                    ...
                    <div className="flex flex-col md:flex-row gap-6">
                         <div className="flex flex-col gap-3 flex-1">
                            {['No', 'Si'].map(opt => (
                                <button key={opt} onClick={() => { setDrugUse(opt); if(opt === 'No') setDrugDetails(''); }} className={`p-4 rounded-xl text-lg font-bold border-2 text-left transition-all ${drugUse === opt ? 'bg-purple-600 text-white border-purple-600' : 'bg-white text-slate-500 border-purple-200'}`}>
                                    {opt}
                                </button>
                            ))}
                         </div>
                         {drugUse === 'Si' && (
                             <div className="flex-1 bg-white p-4 rounded-xl border border-purple-200">
                                 <label className="block font-bold text-purple-900 mb-2">¿Cuál y con qué frecuencia?</label>
                                 <textarea 
            ...
            <div className="animate-fadeIn">
                ...
                <div className="bg-blue-50 p-6 md:p-8 rounded-3xl border-2 border-blue-100 mb-8">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                        ...
                        <button onClick={addMedication} className="bg-blue-600 text-white px-6 py-3 rounded-xl font-bold text-lg hover:bg-blue-700 shadow-lg flex items-center gap-2 transition-transform active:scale-95">
                    ...
                    {medications.length === 0 ? (
                        <div className="text-center py-10 bg-white/50 rounded-2xl border-2 border-dashed border-blue-200">
                            <p className="text-xl text-blue-400 font-medium">No ha agregado medicamentos aún.</p>
                        </div>
                    ) : (
                        <div className="space-y-4">
                            {medications.map((med, idx) => (
                                <div key={med.id} className="bg-white p-6 rounded-2xl shadow-sm border border-blue-100 relative">
                                    <div className="absolute -left-3 top-6 bg-blue-600 text-white w-8 h-8 rounded-full flex items-center justify-center font-bold shadow-md">{idx + 1}</div>
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pl-4">
                                        <div className="md:col-span-3 lg:col-span-1">
                                            ...
                                            <input placeholder="Ej. Losartán" value={med.name} onChange={e => updateMedication(med.id, 'name', e.target.value)} className="w-full text-lg font-bold text-slate-800 border-b-2 border-slate-100 focus:border-blue-500 outline-none py-1" />
                                        </div>
                                        <div>
                                            <label className="text-xs font-bold text-slate-400 uppercase">Dosis</label>
                                            <input placeholder="Ej. 50mg" value={med.dose} onChange={e => updateMedication(med.id, 'dose', e.target.value)} className="w-full text-lg text-slate-800 border-b-2 border-slate-100 focus:border-blue-500 outline-none py-1" />
                                        </div>
                                        <div className="flex items-end gap-2">
                                            <div className="flex-1">
                                                <label className="text-xs font-bold text-slate-400 uppercase">Frecuencia</label>
                                                <input placeholder="Ej. 1 al día" value={med.frequency} onChange={e => updateMedication(med.id, 'frequency', e.target.value)} className="w-full text-lg text-slate-800 border-b-2 border-slate-100 focus:border-blue-500 outline-none py-1" />
                                            </div>
                                            <button onClick={() => removeMedication(med.id)} className="bg-red-100 text-red-600 p-3 rounded-xl hover:bg-red-200">
                ...
                <div className="bg-yellow-50 p-6 md:p-8 rounded-3xl border-2 border-yellow-100">
                    ...
                    <p className="text-yellow-800 text-lg mb-4 font-medium">¿Es alérgico a algún medicamento o comida?</p>
                    <textarea 
        ...
        <div className="bg-slate-50 p-6 md:p-8 border-t border-slate-200 flex flex-col-reverse md:flex-row justify-between items-center gap-4">
            {step === 1 ? (
                <button onClick={onCancel} className="w-full md:w-auto px-8 py-4 text-slate-500 font-bold text-lg hover:bg-slate-200 rounded-xl transition-colors">
                    Cancelar
                </button>
            ) : (
                <button onClick={() => {setStep(step - 1); window.scrollTo(0,0)}} className="w-full md:w-auto px-8 py-4 text-slate-600 font-bold text-lg bg-white border-2 border-slate-200 hover:bg-slate-100 rounded-xl transition-colors flex items-center justify-center gap-2">
                    <ArrowLeft className="w-6 h-6" /> Volver Atrás
                </button>
            )}

            {step < 3 ? (
                <button onClick={handleNext} className="w-full md:w-auto px-12 py-4 bg-blue-600 text-white font-bold text-xl rounded-xl hover:bg-blue-700 shadow-xl shadow-blue-200 transition-all transform active:scale-95 flex items-center justify-center gap-3">
                    Siguiente Paso <ArrowRight className="w-6 h-6" />
                </button>
            ) : (
                <button onClick={handleSave} className="w-full md:w-auto px-12 py-4 bg-green-600 text-white font-bold text-xl rounded-xl hover:bg-green-700 shadow-xl shadow-green-200 transition-all transform active:scale-95 flex items-center justify-center gap-3">
