                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
                    ...
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
                        <div className="flex justify-between items-center mb-4">
                            <button onClick={() => onMonthChange(-1)} className="p-1 hover:bg-white rounded shadow-sm"><ChevronLeft className="w-5 h-5"/></button>
                            <span className="font-bold text-slate-800 uppercase">
                                {currentMonth.toLocaleDateString('es-CL', { month: 'long', year: 'numeric' })}
                            </span>
                            <button onClick={() => onMonthChange(1)} className="p-1 hover:bg-white rounded shadow-sm"><ChevronRight className="w-5 h-5"/></button>
                        </div>
                                ...
                                return (
                                    <button 
                                        ...
                                        className={`
                                            ...
                                            ${isSelected ? 'bg-blue-600 text-white shadow-md scale-105 z-10' : 
                                              isPast ? 'bg-red-50 text-red-300 border border-red-50 cursor-not-allowed' : 
                                              'bg-white text-slate-700 hover:bg-blue-50 border border-slate-100'}
                                        `}
                                    >
                                        <span className="font-bold text-sm">{day.getDate()}</span>
                                        {hasSlots && !isSelected && !isPast && <div className="w-1.5 h-1.5 bg-green-500 rounded-full mt-0.5"></div>}
                                    </button>
                                );
                            })}
                        </div>
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`
                                                ...
                                                ${isPastDate || readOnly ? 'bg-slate-50 border-slate-100 text-slate-300 cursor-not-allowed opacity-60' :
                                                  isBooked 
                                        ...
                                        >
                                            {templateSlot.time}
                                            <span className="text-[10px] uppercase">{isPastDate ? 'Pasado' : isBooked ? 'Paciente' : isOpen ? 'Disponible' : 'Cerrado'}</span>
                                        </button>
                                    )
                                })}
                            </div>
