...
const MiniTrendChart = ({ data, unit, label }: { data: HistoryPoint[], unit?: string, label: string }) => {
    if (!data || data.length === 0) return <p className="text-xs text-slate-400 italic p-2">Sin historial suficiente.</p>;

    // Sort data just in case
    const sortedData = [...data].sort((a,b) => new Date(a.fullDate).getTime() - new Date(b.fullDate).getTime()); // Oldest first
        ...
        <div className="w-64 bg-white border border-slate-200 rounded-xl shadow-xl p-4 text-sm animate-fadeIn z-50">
            <div className="flex justify-between items-center mb-2 border-b border-slate-100 pb-2">
                ...
                <div className="flex items-center gap-1 bg-slate-100 px-2 py-0.5 rounded-full">
                    <span className="font-bold text-slate-800 text-xs">{lastPoint.value} {unit}</span>
                    ...
                    {points.map((p, idx) => (
                        <g key={idx}>
                            <circle cx={p.x} cy={p.y} r="3" fill="white" stroke="#2563eb" strokeWidth="2" />
                             {/* Label Last Point */}
                             {(idx === points.length - 1) && (
                                <text x={p.x} y={p.y - 6} fontSize="9" fill="#2563eb" textAnchor="middle" fontWeight="bold">{p.value}</text>
                            )}
                        </g>
                    ))}
                </svg>
            ...
            <div className="flex justify-between text-[10px] text-slate-400 font-mono">
             ...
             <p className="text-[10px] text-center text-slate-400 mt-1">Última medición: {sortedData[sortedData.length -1].date}</p>
        </div>
    );
};
...
const VitalsInput: React.FC<VitalsInputProps> = ({ 
    label, value, onChange, placeholder, unit, historyData, type="text", readOnly=false, maxLength 
}) => {
    return (
        <div className="relative group/input">
            <div className="flex justify-between items-center mb-2">
                ...
                <div className="group relative inline-block">
                    <label className={`text-sm md:text-base font-bold uppercase tracking-wide truncate pr-2 flex items-center gap-1 cursor-help transition-colors ${historyData && historyData.length > 0 ? 'text-blue-600 hover:text-blue-800' : 'text-slate-600'}`} title={label}>
                        {label}
                        {historyData && historyData.length > 0 && <TrendingUp className="w-3 h-3 text-blue-400 opacity-50 group-hover:opacity-100"/>}
                    </label>
            ...
            <div className="relative">
                ...
                />
                {unit && <span className="absolute right-2 top-4 text-xs text-slate-400 pointer-events-none font-bold">{unit}</span>}
            </div>
        </div>
    );
};
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="bg-blue-50/50 p-8 rounded-3xl border border-blue-100">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
                    <VitalsInput label="Peso" value={newConsultation.weight} onChange={e => onChange('weight', e.target.value)} placeholder="00" unit="kg" historyData={getStandardHistory('weight')} type="number" />
                    <VitalsInput label="Talla" value={newConsultation.height} onChange={e => onChange('height', e.target.value)} placeholder="000" unit="cm" historyData={getStandardHistory('height')} type="number" />
                    ...
                    <div className="bg-white border-2 border-slate-200 rounded-xl flex flex-col justify-center items-center h-[66px] mt-8 shadow-sm">
                        <span className="text-xs text-slate-400 font-bold uppercase tracking-widest mb-1">IMC</span>
                        <span className={`font-bold text-xl leading-none ${Number(newConsultation.bmi) > 25 ? 'text-orange-500' : 'text-slate-800'}`}>{newConsultation.bmi || '-'}</span>
                    </div>
                    
                    {/* BP Chart uses Systolic only for trend */}
                    <VitalsInput label="P. Arterial" maxLength={7} value={newConsultation.bloodPressure} onChange={e => onChange('bloodPressure', e.target.value)} placeholder="120/80" historyData={getBPHistory()} />
                    
                    {/* Show HGT if not Nutritionist (usually they focus on anthro, but can keep it) OR explicitly for diabetic checks */}
                    <VitalsInput label="HGT" value={newConsultation.hgt} onChange={e => onChange('hgt', e.target.value)} placeholder="mg/dl" unit="mg/dl" historyData={getStandardHistory('hgt')} type="number" />
                </div>
                ...
                {showAnthropometry && (
                    <div className="mt-8 pt-6 border-t border-blue-200/50">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                            <VitalsInput label="Circ. Cintura" value={newConsultation.waist} onChange={e => onChange('waist', e.target.value)} placeholder="00" unit="cm" historyData={getStandardHistory('waist')} type="number" />
                            <VitalsInput label="Circ. Cadera" value={newConsultation.hip} onChange={e => onChange('hip', e.target.value)} placeholder="00" unit="cm" historyData={getStandardHistory('hip')} type="number" />
                        </div>
                    </div>
                )}
            </div>
                ...
                <div className="bg-emerald-50/50 p-8 rounded-3xl border border-emerald-100 animate-fadeIn">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        {activeExams.map(examId => {
                            ...
                            if(examId === 'vfg') {
                                return <VitalsInput key={examId} label={config.label} value={displayedVFG} onChange={() => {}} unit={config.unit} readOnly={true} historyData={getExamHistory('vfg')}/>;
                            }
