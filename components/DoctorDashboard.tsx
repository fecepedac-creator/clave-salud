...
export const ProfessionalDashboard: React.FC<ProfessionalDashboardProps> = ({
  ...
  const handleSaveExamOrderProfile = async (profile: { label: string; exams: string[] }) => {
    ...
    try {
      ...
      const updatedProfiles = [...(currentUser.savedExamOrderProfiles || []), newP];
      const updatedDoctor = { ...currentUser, savedExamOrderProfiles: updatedProfiles };

      onUpdateDoctor(updatedDoctor);
      showToast("Pack de exámenes guardado en tu perfil.", "success");
    } catch (e) {
      console.error("Error saving exam profile:", e);
      showToast("Error al guardar el pack", "error");
    }
  };
  ...
  const handleDeleteExamOrderProfile = async (id: string) => {
    ...
    try {
      ...
      console.error("Error deleting exam profile:", e);
      showToast("Error al eliminar el pack", "error");
    }
  };
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen bg-slate-50 transition-all duration-300">
          ...
          >
            ...
            {/* <div data-testid="doctor-tab-bar" className="flex gap-2 bg-white/50 backdrop-blur-sm p-1.5 rounded-xl border border-slate-200/50 w-full md:w-fit shadow-sm overflow-x-auto">
              <button
                ...
                className={`px-3 sm:px-6 py-2 sm:py-2.5 rounded-lg font-bold text-xs sm:text-sm transition-all flex items-center gap-2 whitespace-nowrap ${activeTab === "patients" ? "bg-slate-800 text-white shadow-md" : "text-slate-500 hover:bg-white hover:text-slate-800"}`}
              >
                <UsersRound className="w-4 h-4" /> <span className="hidden xs:inline">Pacientes</span>
              </button>
              <button
                data-testid="doctor-tab-agenda"
                onClick={() => setActiveTab("agenda")}
                className={`px-3 sm:px-6 py-2 sm:py-2.5 rounded-lg font-bold text-xs sm:text-sm transition-all flex items-center gap-2 whitespace-nowrap ${activeTab === "agenda" ? "bg-slate-800 text-white shadow-md" : "text-slate-500 hover:bg-white hover:text-slate-800"}`}
              >
                <CalendarCheck className="w-4 h-4" /> <span className="hidden xs:inline">Agenda</span>
              </button>
              <button
                data-testid="doctor-tab-settings"
                onClick={() => setActiveTab("settings")}
                className={`px-3 sm:px-6 py-2 sm:py-2.5 rounded-lg font-bold text-xs sm:text-sm transition-all flex items-center gap-2 whitespace-nowrap ${activeTab === "settings" ? "bg-slate-800 text-white shadow-md" : "text-slate-500 hover:bg-white hover:text-slate-800"}`}
              >
                <Settings className="w-4 h-4" /> <span className="hidden xs:inline">Config</span>
              </button>
              <button
                data-testid="doctor-tab-performance"
                onClick={() => setActiveTab("performance")}
                className={`px-3 sm:px-6 py-2 sm:py-2.5 rounded-lg font-bold text-xs sm:text-sm transition-all flex items-center gap-2 whitespace-nowrap ${activeTab === "performance" ? "bg-slate-800 text-white shadow-md" : "text-slate-500 hover:bg-white hover:text-slate-800"}`}
              >
      ...
      <a
        ...
        className="fixed bottom-4 right-4 bg-slate-800 text-white p-3 sm:px-4 sm:py-2 rounded-full shadow-lg hover:bg-slate-900 transition-colors flex items-center gap-2 z-50 text-xs sm:text-sm font-medium"
        target="_blank"
        rel="noreferrer"
