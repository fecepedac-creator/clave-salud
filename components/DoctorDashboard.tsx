...
// Edit Mode State
  ...
  const [tempTemplate, setTempTemplate] = useState<ClinicalTemplate>({ id: '', title: '', content: '' });
  ...
  const [tempProfile, setTempProfile] = useState<ExamProfile>({ id: '', label: '', exams: [], description: '' });
  const [isEditingProfileId, setIsEditingProfileId] = useState<string | null>(null);

  // Custom Exams State
  const [newCustomExam, setNewCustomExam] = useState<Partial<ExamDefinition>>({ label: '', unit: '', category: '' });
...
useEffect(() => {
  ...
  const filteredPatients = patients.filter(p => {
      ...
      const nameMatch = p.fullName.toLowerCase().includes(searchTerm.toLowerCase()) || p.rut.includes(searchTerm);
  ...
  const handleSaveTemplate = () => {
    ...
    if (isEditingTemplateId) {
        updatedTemplates = myTemplates.map(t => t.id === isEditingTemplateId ? { ...tempTemplate, id: isEditingTemplateId } : t);
  ...
  const handleDeleteCustomExam = (examId: string) => {
      if(window.confirm("¿Eliminar este examen personalizado?")) {
          ...
          setMyExamProfiles(updatedProfiles);
          onUpdateDoctor({ id: doctorId, savedExamProfiles: updatedProfiles, customExams: updatedCustoms } as any);
      }
  };
  ...
  const canSeeVitals = ['Medico', 'Enfermera', 'Kinesiologo', 'Matrona', 'Nutricionista'].includes(role);
          ...
          <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 text-sm text-slate-500 font-medium">
                    <span>{calculateAge(selectedPatient.birthDate)} años • {selectedPatient.gender}</span>
                ...
                <PatientSidebar 
                    ...
                    toggleEditPatient={() => { 
                        if(isEditingPatient) { 
                            ...
                            // LOG ACTIVITY
                            onLogActivity('update', `Actualizó datos ficha de ${selectedPatient.fullName}`, selectedPatient.id);
                    ...
                    onFileUpload={(e) => { 
                        if(e.target.files?.[0]) {
                            ...
                            const att: Attachment = { id: generateId(), name: f.name, type: f.type.includes('image')?'image':'pdf', date: new Date().toISOString(), url: URL.createObjectURL(f) };
                            const up = {...selectedPatient, attachments: [...(selectedPatient.attachments || []), att]}; // SAFE
                            onUpdatePatient(up); 
                            setSelectedPatient(up);
                            onLogActivity('update', `Subió archivo ${f.name} a paciente ${selectedPatient.fullName}`, selectedPatient.id);
                        }
                    }}
                         ...
                         <div className="flex justify-between items-end mb-8">
                             <div>
                                 ...
                                 <p className="text-slate-500 text-base mt-1 flex items-center gap-2">
                                     ...
                                     {selectedPatient.consultations ? selectedPatient.consultations.length : 0} atenciones registradas
                                 </p>
                             </div>
                             {!isReadOnly && (
                                  <button
                             ...
                             <div className="p-8 md:p-10 space-y-10">
                                 ...
                                 {isDentist && moduleGuards.dental && (
                                     <Odontogram 
                                        ...
                                        onChange={(val) => setNewConsultation({...newConsultation, dentalMap: val})}
                                     />
                                 )}
                                 ...
                                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                     <div className="col-span-full">
                                         <label className="block text-lg font-bold text-slate-700 mb-3">Motivo de Consulta</label>
                                         <input value={newConsultation.reason} onChange={e => setNewConsultation({...newConsultation, reason: e.target.value})} className="w-full p-4 border-2 border-slate-200 rounded-xl focus:ring-4 focus:ring-primary-100 focus:border-primary-500 outline-none font-medium text-xl text-slate-800" />
                                     </div>
                                     <div className={isPsych ? "col-span-full" : ""}>
                                         <label className="block text-lg font-bold text-slate-700 mb-3">{isPsych ? 'Desarrollo de la Sesión / Evolución' : 'Anamnesis Próxima'}</label>
                                         <textarea value={newConsultation.anamnesis} onChange={e => setNewConsultation({...newConsultation, anamnesis: e.target.value})} className="w-full p-4 border-2 border-slate-200 rounded-xl focus:ring-4 focus:ring-primary-100 focus:border-primary-500 outline-none resize-none h-60 text-lg leading-relaxed text-slate-700" />
                                     </div>
                                     {!isPsych && (
                                         <div>
                                             <label className="block text-lg font-bold text-slate-700 mb-3">Examen Físico</label>
                                             <textarea value={newConsultation.physicalExam} onChange={e => setNewConsultation({...newConsultation, physicalExam: e.target.value})} className="w-full p-4 border-2 border-slate-200 rounded-xl focus:ring-4 focus:ring-primary-100 focus:border-primary-500 outline-none resize-none h-60 text-lg leading-relaxed text-slate-700" />
                                         </div>
                                     )}
                                     <div className="col-span-full">
                                         <label className="block text-lg font-bold text-slate-700 mb-3">Diagnóstico / Hipótesis</label>
                                         <AutocompleteInput 
                                            value={newConsultation.diagnosis || ''} 
                                            onChange={(val) => setNewConsultation({...newConsultation, diagnosis: val})}
    ...
    <>
      <PrescriptionManager
        ...
        onRemovePrescription={(id) =>
          setNewConsultation({
            ...newConsultation,
            prescriptions: newConsultation.prescriptions?.filter((p) => p.id !== id),
          })
        }
        onPrint={(docs) => {
          setDocsToPrint(docs);
      ...
      {!canPrescribeDrugs && (
        <p className="text-xs text-slate-400 mt-2 italic">
          * Su perfil no permite emitir recetas de medicamentos o exámenes, solo indicaciones y certificados.
        </p>
      )}
    </>
  ) : (
    <div className="p-4 rounded-2xl border border-slate-200 bg-slate-50 text-slate-500">
      Módulo de Indicaciones/Recetas deshabilitado por el administrador del centro.
    </div>
  )}                                   </div>
                                 
                                 {/* 5. Next Control */}
                                 <div className="bg-secondary-50 p-8 rounded-3xl border border-secondary-100">
                                     <h4 className="text-secondary-900 font-bold text-lg uppercase tracking-wider mb-6 flex items-center gap-2"><Calendar className="w-5 h-5"/> Próximo Control</h4>
                                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                         <div>
                                             <label className="block text-lg font-bold text-slate-700 mb-3">Fecha Estimada</label>
                                             <input type="date" value={newConsultation.nextControlDate} onChange={e => setNewConsultation({...newConsultation, nextControlDate: e.target.value})} className="w-full p-4 border-2 border-secondary-200 rounded-xl outline-none focus:border-secondary-500 bg-white text-lg" />
                                         </div>
                                         <div>
                                             <label className="block text-lg font-bold text-slate-700 mb-3">Indicaciones / Requisitos</label>
                                             <input placeholder="Ej: Traer radiografía..." value={newConsultation.nextControlReason} onChange={e => setNewConsultation({...newConsultation, nextControlReason: e.target.value})} className="w-full p-4 border-2 border-secondary-200 rounded-xl outline-none focus:border-secondary-500 bg-white text-lg" />
                                         </div>
                                     </div>
                                 </div>
                                 ...
                                 <div className="flex justify-end items-center pt-8 border-t border-slate-100 gap-4 relative">
                                     {canIssueLicense && (
                                        <div className="relative">
                                            <button onClick={() => setShowLicenciaOptions(!showLicenciaOptions)} className="text-primary-600 font-bold text-lg hover:bg-primary-50 px-6 py-3 rounded-xl transition-colors border border-primary-200">
                                            ...
                                            {showLicenciaOptions && (
                                                <div className="absolute bottom-full right-0 mb-2 bg-white border border-slate-200 shadow-xl rounded-xl p-4 w-72 animate-fadeIn z-20">
                                                    <a href="https://wlme.medipass.cl" target="_blank" className="flex items-center gap-3 p-3 hover:bg-slate-50 rounded-xl text-slate-700 font-medium"><ExternalLink className="w-4 h-4"/> Medipass</a>
                                                    <a href="https://www.licencia.cl" target="_blank" className="flex items-center gap-3 p-3 hover:bg-slate-50 rounded-xl text-slate-700 font-medium"><ExternalLink className="w-4 h-4"/> I-Med</a>
                                                </div>
                                            )}
                                        </div>
                                     )}
              ...
              <div className={`max-w-7xl mx-auto w-full h-full flex flex-col ${activeTab === 'settings' ? '' : 'overflow-hidden'}`}>
                  ...
                  <div className="flex-shrink-0 px-8 pt-8 pb-4">
                      <div className="flex gap-2 bg-white/50 backdrop-blur-sm p-1.5 rounded-xl border border-slate-200/50 w-fit shadow-sm">
                          <button onClick={() => setActiveTab('patients')} className={`px-6 py-2.5 rounded-lg font-bold text-sm transition-all flex items-center gap-2 ${activeTab === 'patients' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-500 hover:bg-white hover:text-slate-800'}`}>
                              <UsersRound className="w-4 h-4"/> Pacientes
                          </button>
                          <button onClick={() => setActiveTab('agenda')} className={`px-6 py-2.5 rounded-lg font-bold text-sm transition-all flex items-center gap-2 ${activeTab === 'agenda' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-500 hover:bg-white hover:text-slate-800'}`}>
                              <CalendarCheck className="w-4 h-4"/> Mi Agenda
                          </button>
                          <button onClick={() => setActiveTab('settings')} className={`px-6 py-2.5 rounded-lg font-bold text-sm transition-all flex items-center gap-2 ${activeTab === 'settings' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-500 hover:bg-white hover:text-slate-800'}`}>
                  ...
                  {/* CONTENT AREA */}
                  <div className={`flex-1 px-8 pb-8 ${activeTab === 'settings' ? 'overflow-y-auto' : 'overflow-hidden'}`}>
                              ...
                              <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-white/50 flex-shrink-0">
                                  ...
                                  <div className="flex items-center gap-3">
                                      <div className="flex bg-white rounded-lg border border-slate-200 p-1">
                                          <button onClick={() => setFilterNextControl('all')} className={`px-3 py-1.5 rounded-md text-xs font-bold transition-colors ${filterNextControl === 'all' ? 'bg-slate-100 text-slate-700' : 'text-slate-400 hover:text-slate-600'}`}>Todos</button>
                                          <button onClick={() => setFilterNextControl('week')} className={`px-3 py-1.5 rounded-md text-xs font-bold transition-colors ${filterNextControl === 'week' ? 'bg-slate-100 text-slate-700' : 'text-slate-400 hover:text-slate-600'}`}>Control 7d</button>
                                          <button onClick={() => setFilterNextControl('month')} className={`px-3 py-1.5 rounded-md text-xs font-bold transition-colors ${filterNextControl === 'month' ? 'bg-slate-100 text-slate-700' : 'text-slate-400 hover:text-slate-600'}`}>Control 30d</button>
                                      </div>
                                      {!isReadOnly && (
                                          <button 
                                              onClick={() => {
                                                  ...
                                                  const newP: Patient = { 
                                                      ...
                                                      medicalHistory: [], surgicalHistory: [], medications: [], allergies: [], consultations: [], attachments: [], livingWith: [], smokingStatus: 'No fumador', alcoholStatus: 'No consumo', lastUpdated: new Date().toISOString()
                                                  };
                                      ...
                                      <table className="w-full text-left border-collapse">
                                          ...
                                          <tbody className="divide-y divide-slate-100">
                                              {filteredPatients.map(p => {
                                                  const lastConsult = p.consultations && p.consultations.length > 0 ? p.consultations[0] : null;
                                                  ...
                                                  return (
                                                      <tr key={p.id} className="hover:bg-slate-50/80 transition-colors group cursor-pointer" onClick={() => setSelectedPatient(p)}>
                                                          ...
                                                          <td className="p-5 hidden md:table-cell">
                                                              <div className="text-slate-600 font-medium">{calculateAge(p.birthDate)} años</div>
                                                          ...
                                                          <td className="p-5">
                                                              {nextCtrl ? (
                                                                  <span className={`px-3 py-1 rounded-full text-xs font-bold ${isControlNear ? 'bg-orange-100 text-orange-700' : 'bg-green-50 text-green-700'}`}>
                      ...
                      {activeTab === 'settings' && moduleGuards.settings && (
                          <div className="w-full animate-fadeIn grid grid-cols-1 lg:grid-cols-12 gap-8 pb-20">
                              ...
                              <div className="lg:col-span-6 bg-white/90 backdrop-blur-sm p-8 rounded-3xl border border-white shadow-lg flex flex-col h-[600px]">
                                  <h3 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2"><Layers className="w-6 h-6 text-emerald-500"/> Mis Perfiles de Exámenes</h3>
                                              ...
                                              <div key={profile.id} className="p-4 rounded-xl border border-slate-100 bg-slate-50 hover:bg-white hover:border-emerald-200 transition-all shadow-sm group">
                                                  ...
                                                  <div className="flex flex-wrap gap-1">
                                                      {profile.exams.map(ex => {
                                                          ...
                                                          return <span key={ex} className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded font-bold uppercase">{label}</span>
                                                      })}
                                                  </div>
                                              </div>
                                          ))}
                                          ...
                                          <div className="space-y-3 bg-slate-50 p-4 rounded-xl border border-slate-200">
                                              <div className="flex gap-2">
                                                  <div className="flex-1 space-y-1">
                                                      <label className="text-xs font-bold text-slate-400 uppercase">Nombre Perfil</label>
                                                      <input 
                                              ...
                                              {/* Checkbox Grid */}
                                              <div>
                                                  <label className="text-xs font-bold text-slate-400 uppercase block mb-1">Seleccionar Exámenes</label>
                                              ...
                                              <div className="flex gap-2 pt-2">
                                                  <button onClick={handleSaveProfile} className="flex-1 bg-emerald-600 text-white font-bold py-2 rounded-lg hover:bg-emerald-700 transition-colors text-sm">
                                                      {isEditingProfileId ? 'Guardar Cambios' : 'Crear Perfil'}
                                                  </button>
                                                  {isEditingProfileId && (
                                                      <button onClick={() => { setIsEditingProfileId(null); setTempProfile({id:'', label:'', exams:[], description:''}); }} className="px-3 py-2 bg-slate-200 text-slate-600 font-bold rounded-lg hover:bg-slate-300 text-sm">
                                                          Cancelar
                                                      </button>
                                                  )}
                                              </div>
                                              <button onClick={handleResetProfiles} className="w-full text-xs text-slate-400 hover:text-emerald-600 flex justify-center items-center gap-1 mt-1">
                              ...
                              <div className="lg:col-span-6 space-y-8">
                                  ...
                                  <div className="bg-white/90 backdrop-blur-sm p-8 rounded-3xl border border-white shadow-lg flex flex-col h-auto">
                                      ...
                                      <div className="bg-purple-50 p-4 rounded-xl border border-purple-100 space-y-3">
                                          <p className="text-xs text-purple-700 mb-2">Cree un examen personalizado si no está en la lista estándar.</p>
                                          <div className="flex gap-2">
                                              <input 
                                      ...
                                      {currentUser?.customExams && currentUser.customExams.length > 0 && (
                                          <div className="mt-4 border-t border-slate-100 pt-4">
                                              <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">Mis Exámenes Personalizados</h4>
                                  ...
                                  <div className="bg-white/90 backdrop-blur-sm p-8 rounded-3xl border border-white shadow-lg flex flex-col min-h-[300px]">
                                      <h3 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2"><FileText className="w-6 h-6 text-slate-400"/> Mis Plantillas Clínicas</h3>
                                      
                                      <div className="flex-1 overflow-hidden flex flex-col gap-6">
                                          {/* List */}
                                          <div className="flex-1 overflow-y-auto pr-2 space-y-2 border-b border-slate-100 pb-4 max-h-40">
                                              {myTemplates.length === 0 && <p className="text-center text-slate-400 text-sm italic py-4">No tiene plantillas.</p>}
                                              {myTemplates.map(t => (
                                          ...
                                          <div className="space-y-3 bg-slate-50 p-4 rounded-xl border border-slate-200">
                                              ...
                                              {!isReadOnly && (
                                                  <div className="flex gap-2">
                                                      <button onClick={handleSaveTemplate} className="flex-1 bg-blue-600 text-white font-bold py-2 rounded-lg hover:bg-blue-700 transition-colors text-sm">
                                                          {isEditingTemplateId ? 'Actualizar' : 'Crear Nueva'}
                                                      </button>
                                                      {isEditingTemplateId && (
                                                          <button onClick={() => { setIsEditingTemplateId(null); setTempTemplate({id:'', title:'', content:''}); }} className="px-3 py-2 bg-slate-200 text-slate-600 font-bold rounded-lg hover:bg-slate-300 text-sm">
                                                              Cancelar
                                                          </button>
                                                      )}
                                                  </div>
                                  ...
                                  <div className="flex flex-col md:flex-row gap-8">
                                      <div className="md:w-1/3">
                                          <div className="bg-indigo-50 p-6 rounded-2xl border border-indigo-100 text-indigo-900">
                                              ...
                                              <p className="text-sm opacity-80 mb-4">Es recomendable actualizar su contraseña periódicamente para proteger el acceso a las fichas clínicas.</p>
                                              <p className="text-xs font-bold uppercase tracking-wider opacity-60">Requisitos</p>
                                      ...
                                      <div className="flex-1 space-y-4 max-w-md">
                                          ...
                                          <div className="grid grid-cols-2 gap-4">
                                              ...
                                              <div>
                                                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Repetir Nueva</label>
                                                  <input 
                                                      type="password" 
                                                      className="w-full p-3 border rounded-xl outline-none focus:border-indigo-500 bg-slate-50 focus:bg-white transition-colors"
                                                      placeholder="••••••••"
