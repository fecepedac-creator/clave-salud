...
const Breadcrumbs: React.FC<BreadcrumbsProps> = ({ view, centerName, onNavigate }) => {
    const getBreadcrumbs = () => {
        const items = [
            { label: 'Inicio', view: 'home' as ViewMode, icon: Home }
        ];
        ...
        if (view === 'doctor-dashboard') {
            ...
            items.push({ label: 'Seleccionar Centro', view: 'select-center' as ViewMode, icon: Building2 });
        }
                ...
                {items.map((item, index) => (
                    <React.Fragment key={`${item.view}-desktop-${index}`}>
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1.5 sm:gap-2 hover:text-sky-600 transition-colors px-1 sm:px-2 py-1 rounded-lg hover:bg-sky-50/50 ${index === items.length - 1 ? 'font-extrabold text-slate-800' : 'font-medium'
                                }`}
