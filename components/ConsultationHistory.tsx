    ...
    <div className="space-y-6">
                        ...
                        {(correctionsByConsultation[c.id] || []).map((correction) => (
                          ...
                          >
                            ...
                            <p className="mt-2 text-xs text-slate-500">
                              {correction.professionalName || "Profesional"} ·{" "}
                              {correctionDate(correction.createdAt)}
                            </p>
                          </div>
                        ))}
                      </div>
      ...
      {correctionTarget && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Agregar corrección clínica"
        >
          <div className="w-full max-w-xl rounded-3xl bg-white p-6 shadow-2xl">
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  La atención original no se modifica. Esta nota queda vinculada y auditada.
                </p>
              </div>
              <button
                type="button"
            ...
            <select
              ...
              onChange={(event) =>
                setCorrectionKind(event.target.value as ClinicalCorrection["kind"])
              }
            ...
            <textarea
              ...
              maxLength={10000}
              rows={7}
              placeholder="Describa con precisión qué información se agrega o corrige."
              className="w-full resize-y rounded-xl border border-slate-200 p-3 text-slate-800 outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-100"
            />
            <div className="mt-5 flex justify-end gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-amber-600 px-4 py-2.5 font-bold text-white hover:bg-amber-700 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {isSavingCorrection && <Loader2 className="h-4 w-4 animate-spin" />} Guardar
                corrección
              </button>
            </div>
          </div>
