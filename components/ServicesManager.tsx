...
const CATEGORY_LABELS = {
    LABORATORY: { label: "Laboratorio", icon: FlaskConical, color: "text-blue-400", bg: "bg-blue-500/10" },
    IMAGING: { label: "Imagenología", icon: ImageIcon, color: "text-purple-400", bg: "bg-purple-500/10" },
    CARDIOLOGY: { label: "Cardiología", icon: HeartPulse, color: "text-red-400", bg: "bg-red-500/10" },
    PROCEDURE: { label: "Procedimiento", icon: Stethoscope, color: "text-emerald-400", bg: "bg-emerald-500/10" },
...
const ServicesManager: React.FC<ServicesManagerProps> = ({ centerId }) => {
    ...
    useEffect(() => {
        ...
        const q = query(
            collection(db, "centers", centerId, "services"),
            orderBy("name", "asc")
        );
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            showToast(`Prestación ${currentService.id ? "actualizada" : "creada"} correctamente.`, "success");
            setIsEditing(false);
            setCurrentService({
                name: "",
                category: "LABORATORY",
    ...
    const handleActivateBundle = async (bundle: typeof PREDEFINED_BUNDLES[0]) => {
        if (!window.confirm(`¿Desea cargar automáticamente el ${bundle.title}? Se añadirán ${bundle.services.length} prestaciones base.`)) return;
            ...
            <div className="bg-slate-800 p-8 rounded-3xl border border-slate-700">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div>
                        ...
                        <p className="text-slate-400 mt-2">Administra los exámenes, procedimientos y servicios del centro.</p>
                    </div>
                    <button
                        onClick={() => {
                            setCurrentService({
                ...
                {services.length > 0 && !loading && (
                    <div className="mb-6">
                        ...
                        >
                            <Plus className={`w-4 h-4 transition-transform ${showQuickStart ? "rotate-45" : ""}`} />
                ...
                {(services.length === 0 || showQuickStart) && !loading && (
                    <div className="mb-8 p-6 bg-indigo-500/5 border border-indigo-500/20 rounded-2xl animate-pulse-subtle">
                        ...
                        <p className="text-sm text-slate-400 mb-6">Activa módulos predefinidos con los exámenes más comunes para comenzar de inmediato.</p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {PREDEFINED_BUNDLES.map((bundle) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-3 bg-slate-800 group-hover:bg-indigo-500/10 transition-colors`}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-4">
                        {loading ? (
                            ...
                            filteredServices.map((service) => {
                                const CatInfo = CATEGORY_LABELS[service.category as keyof typeof CATEGORY_LABELS] || CATEGORY_LABELS.OTHER;
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center gap-4">
                                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${CatInfo.bg}`}>
                    ...
                    {isEditing ? (
                        <div className="bg-slate-900/70 p-6 rounded-2xl border border-indigo-500/30 h-fit sticky top-4">
                            <div className="flex items-center justify-between mb-6">
                                <h4 className="font-bold text-white text-lg flex items-center gap-2">
                                    {currentService.id ? <Edit className="w-5 h-5 text-indigo-400" /> : <Plus className="w-5 h-5 text-indigo-400" />}
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase">Instrucciones de Preparación</label>
                                    <textarea
                                        rows={3}
                                        ...
                                        onChange={(e) => setCurrentService({ ...currentService, preparationInstructions: e.target.value })}
                                    />
                                ...
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase">Descripción (Interna)</label>
                                    <input
                                        className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white outline-none focus:border-indigo-500 text-sm"
                                ...
                                <div className="p-4 bg-slate-800 rounded-xl border border-slate-700">
                                    <label className="text-xs font-bold text-slate-500 uppercase block mb-3">Documento de Preparación (PDF/Imagen)</label>
                                    {currentService.instructionsFile ? (
                                        <div className="flex items-center justify-between bg-indigo-500/10 border border-indigo-500/30 p-2 rounded-lg">
                                            <div className="flex items-center gap-2 overflow-hidden">
                                                ...
                                                <span className="text-xs text-indigo-200 truncate">{currentService.instructionsFileName || "instrucciones.pdf"}</span>
                                            </div>
                                            <button
                                                onClick={() => setCurrentService({ ...currentService, instructionsFile: undefined, instructionsFileName: undefined })}
                                        ...
                                        <label className="flex flex-col items-center justify-center border-2 border-dashed border-slate-700 rounded-xl p-4 hover:border-indigo-500 transition-colors cursor-pointer group">
                                            ...
                                            <span className="text-xs text-slate-500 group-hover:text-slate-300">Cargar preparación</span>
                                ...
                                <label className="flex items-center gap-3 p-3 bg-slate-800 rounded-xl border border-slate-700 hover:border-slate-500 cursor-pointer transition-all">
                                    <input
                                        ...
                                        onChange={(e) => setCurrentService({ ...currentService, isAgendable: e.target.checked })}
                                        className="w-4 h-4 accent-indigo-500"
                                    />
                                    <div>
                                        <span className="block text-sm font-bold text-white">Habilitar Agenda</span>
                                        <span className="block text-[10px] text-slate-500 text-xs italic">Permite agendar horas para este servicio.</span>
                                    </div>
                                </label>
                        ...
                        <div className="hidden lg:flex flex-col items-center justify-center p-8 bg-slate-900/30 rounded-2xl border border-dashed border-slate-700 text-slate-500">
                            ...
                            <p className="text-center text-sm">Selecciona una prestación para editarla o crea una nueva.</p>
                        </div>
                    )}
                </div>
            </div>
