...
const ServiceAgendasManager: React.FC<ServiceAgendasManagerProps> = ({
    ...
    const handleDelete = async (id: string) => {
        if (!window.confirm("¿Eliminar este perfil de servicio? Se perderá su configuración de agenda.")) return;
        ...
        <div className="bg-slate-800 p-8 rounded-3xl border border-slate-700 mt-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="text-slate-400 mt-2">Crea perfiles para gestionar cupos de Laboratorio, Imagenología, etc.</p>
                </div>
                {!isEditing && (
                    <button
                    ...
                    <div className="bg-slate-900/70 p-6 rounded-2xl border border-amber-500/30 h-fit">
                        <div className="flex items-center justify-between mb-6">
                            <h4 className="font-bold text-white text-lg flex items-center gap-2">
                                {currentProfile.id ? <Settings className="w-5 h-5 text-amber-400" /> : <Plus className="w-5 h-5 text-amber-400" />}
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="text-[10px] font-bold text-slate-500 uppercase">Nombre del Servicio / Recurso</label>
                                <input
                            ...
                            <div>
                                <label className="text-[10px] font-bold text-slate-500 uppercase">Descripción / Especialidad</label>
                                <input
                                    className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white outline-none focus:border-amber-500"
                                    value={currentProfile.specialty || ""}
                                    onChange={(e) => setCurrentProfile({ ...currentProfile, specialty: e.target.value })}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-[10px] font-bold text-slate-500 uppercase">Duración Bloque (min)</label>
                                    <input
                                        type="number"
                                        className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white outline-none focus:border-amber-500"
                                        ...
                                        onChange={(e) => setCurrentProfile({
                                            ...currentProfile,
                                            agendaConfig: { ...currentProfile.agendaConfig!, slotDuration: parseInt(e.target.value) }
                                        })}
                                    />
                                </div>
                                <div className="flex flex-col gap-2">
                                    <span className="text-[10px] font-bold text-slate-500 uppercase">Agendamiento Público</span>
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            type="checkbox"
                                            checked={currentProfile.visibleInBooking}
                                            onChange={(e) => setCurrentProfile({ ...currentProfile, visibleInBooking: e.target.checked })}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-[10px] font-bold text-slate-500 uppercase">Inicia Agenda</label>
                                    <input
                                        type="time"
                                        className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white outline-none focus:border-amber-500"
                                        ...
                                        onChange={(e) => setCurrentProfile({
                                            ...currentProfile,
                                            agendaConfig: { ...currentProfile.agendaConfig!, startTime: e.target.value }
                                        })}
                                    />
                                </div>
                                <div>
                                    <label className="text-[10px] font-bold text-slate-500 uppercase">Termina Agenda</label>
                                    <input
                                        type="time"
                                        className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white outline-none focus:border-amber-500"
