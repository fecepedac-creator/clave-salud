...
const PrescriptionManager: React.FC<PrescriptionManagerProps> = ({ prescriptions, onAddPrescription, onRemovePrescription, onPrint, templates, role }) => {
    ...
    const allDocTypes = [
        ...
        { value: "Receta Retenida", label: "Receta Retenida (Controlados)", restricted: true, controlled: true },
                    ...
                    <div className="relative">
                        ...
                        {showTemplateSelector && templates && (
                            <div className="absolute top-full right-0 mt-2 w-64 bg-white border border-slate-200 shadow-xl rounded-xl z-20 overflow-hidden animate-fadeIn">
                                <div className="p-2 bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase">Seleccionar Plantilla</div>
                                <div className="max-h-60 overflow-y-auto">
                                    ...
                                    {templates.length === 0 && <div className="p-4 text-sm text-slate-400 italic">No hay plantillas configuradas.</div>}
                                </div>
                            </div>
                        )}
                        {showTemplateSelector && (
                            <div className="fixed inset-0 z-10" onClick={() => setShowTemplateSelector(false)}></div>
                        )}
                    </div>
                </div>

                <textarea 
                    placeholder={canPrescribe ? "Escriba aquí los fármacos, indicaciones o el contenido del documento..." : "Escriba aquí las indicaciones, certificado o contenido..."}
            ...
            {prescriptions && prescriptions.length > 0 ? (
                <div className="space-y-3">
                    {prescriptions.map(doc => (
                        <div key={doc.id} className="bg-white p-4 rounded-xl border border-amber-200 shadow-sm flex justify-between items-start gap-4">
                            <div className="flex-1">
                                <span className="inline-block bg-amber-100 text-amber-800 text-xs font-bold px-2 py-1 rounded mb-2 uppercase">{doc.type}</span>
