...
const Odontogram: React.FC<OdontogramProps> = ({ value, onChange, readOnly = false }) => {
    ...
    const renderTooth = (id: number) => {
        ...
        return (
            <button 
                ...
                `}
            >
                <div className={`w-8 h-8 md:w-10 md:h-10 rounded border border-slate-300 shadow-sm mb-1 ${conditionConfig?.color} ${data?.status === 'Ausente' ? 'bg-slate-800' : ''}`}></div>
    ...
    return (
        <div className="bg-white p-4 rounded-xl border border-slate-200">
            <h4 className="font-bold text-slate-700 mb-4 text-center uppercase tracking-widest">Odontograma (Adulto)</h4>
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm animate-slideUp">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={() => setSelectedTooth(null)}><X className="w-5 h-5 text-slate-400"/></button>
                        </div>
                        ...
                        <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Observaciones</label>
                        <textarea 
