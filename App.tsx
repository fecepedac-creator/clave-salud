...
const SUPERADMIN_EMAILS = ["fecepedac@gmail.com", "dr.felipecepeda@gmail.com"];
...
const App: React.FC = () => {
  ...
  const handleSuperAdminLogin = async (targetView: ViewMode) => {
    if (targetView === ("superadmin-dashboard" as ViewMode)) {
      setError("Acceso exclusivo con Google.");
      showToast("Usa Google para acceder a SuperAdmin.", "warning");
      return;
    }
    try {
      setError("");
  ...
  const handleGoogleLogin = async (targetView: ViewMode) => {
    try {
      ...
      if (targetView === ("superadmin-dashboard" as ViewMode)) {
        const allowed = SUPERADMIN_EMAILS.includes(emailUser.toLowerCase());
  ...
  const renderCenterPortal = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center p-4 min-h-[calc(100vh-80px)]">
        ...
        <button
          type="button"
          onClick={() => handleGoogleLogin("superadmin-dashboard" as ViewMode)}
