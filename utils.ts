...
/**
 * Formatea un nombre de persona con iniciales mayúsculas (Title Case)
 * Ejemplo: "JUAN PABLO PEREZ" -> "Juan Pablo Perez"
 * Ejemplo: "maría josé gonzález" -> "María José González"
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    if (file.type === 'application/pdf') {
        ...
        if (file.size > MAX_PDF_SIZE) {
            reject(new Error("El PDF es muy pesado. Si es un escaneo, por favor suba fotos (JPG) en su lugar, o comprima el PDF a menos de 750KB."));
            return;
        }
...
export function normalizeRole(role: unknown): CanonicalRole | null {
  ...
  // Canonical
  if (r === "super_admin" || r === "center_admin" || r === "admin" || r === "doctor") return r as CanonicalRole;
...
export function hasRole(roles: AnyRole[] | CanonicalRole[] | undefined | null, required: CanonicalRole): boolean {
