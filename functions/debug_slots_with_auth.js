...
async function debugSlots() {
    ...
    const snap = await db.collection("centers").doc(centerId)
    ...
    snap.forEach(doc => {
        const data = doc.data();
        console.log(`- Time: ${data.time}, Status: ${data.status}, Active: ${data.active}, DoctorId: ${data.doctorId}, DocID: ${doc.id}`);
    });
}
