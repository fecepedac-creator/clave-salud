...
function getFirebaseToken() {
  try {
    const token = execSync("firebase login:ci --no-localhost 2>/dev/null || echo ''", {
      encoding: "utf8",
    }).trim();
    return token || null;
  } catch {
...
async function migratePatientsForCenter(centerId) {
  ...
  console.log(
    `  Default owner UID: ${defaultOwnerUid} (${defaultOwner?.displayName || defaultOwner?.name || "unknown"})`
  );
  ...
  for (const patientDoc of patientsSnap.docs) {
    ...
    if (existingRoot.exists) {
      console.log(
        `  ⏭️  Patient ${patientId} (${patientData.fullName || "?"}) already exists in root — skipping`
      );
      skipped++;
      continue;
    }
    ...
    if (DRY_RUN) {
      console.log(
        `  [DRY] Would migrate patient ${patientId} (${patientData.fullName || "?"}) → owner: ${ownerUid}`
      );
    } else {
      await db.collection("patients").doc(patientId).set(migratedPatient);
      console.log(
        `  ✅ Migrated patient ${patientId} (${patientData.fullName || "?"}) → owner: ${ownerUid}`
      );
    }
    migratedPatients++;
