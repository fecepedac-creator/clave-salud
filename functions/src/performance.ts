...
export const updateAppointmentAttendance = functions.https.onCall(async (data, context) => {
  ...
  const isSuperAdmin =
    context.auth?.token?.super_admin === true || context.auth?.token?.superadmin === true;
  if (!uid && !isSuperAdmin) {
    throw new functions.https.HttpsError("unauthenticated", "Debe estar autenticado.");
  }

  const { centerId, appointmentId, payload } = data as {
    centerId: string;
  ...
  if (!isSuperAdmin) {
    ...
    if (!isCenterAdmin && uid !== professionalId) {
      throw new functions.https.HttpsError(
        "permission-denied",
        "Solo el administrador o el profesional asignado puede modificar esta cita."
      );
    }
  }
  ...
  const closureRef = db
    .collection("centers")
    .doc(centerId)
    .collection("closures_month")
    .doc(yearMonth);
  const closureSnap = await closureRef.get();
  if (closureSnap.exists && closureSnap.data()?.status === "closed") {
    throw new functions.https.HttpsError(
      "failed-precondition",
      `El mes ${yearMonth} ya se encuentra cerrado. No se pueden modificar cobros ni estados de asistencia.`
    );
  }
  ...
  const hasChanges =
    oldStatus !== payload.attendanceStatus ||
  ...
  .onWrite(async (change, context) => {
    ...
    if (change.before.exists && oldYearMonth) {
      ...
      if (oldProfessionalId)
        accumulateMap(profUpdates, `${oldYearMonth}|${oldProfessionalId}`, oldDeltas);
    }

    if (change.after.exists && newYearMonth) {
      accumulateMap(centerUpdates, newYearMonth, newDeltas);
      if (newProfessionalId)
        accumulateMap(profUpdates, `${newYearMonth}|${newProfessionalId}`, newDeltas);
    }
    ...
    for (const [key, d] of profUpdates.entries()) {
      ...
      dbBatch.set(
        ref,
        { id: profKey, centerId, doctorId: profId, yearMonth: ym, ...updateObject },
        { merge: true }
      );
    }
    ...
    console.log(
      `Re-calculated stats for ${centerId} - Appointment: ${context.params.appointmentId}`
    );
  });
...
export const recomputeMonthStats = functions.https.onCall(async (data, context) => {
  ...
  if (!centerId || !yearMonth) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "centerId y yearMonth son requeridos."
    );
  }
  ...
  if (!isSuperAdmin && closureSnap.exists && closureSnap.data()?.status === "closed") {
    throw new functions.https.HttpsError(
      "failed-precondition",
      `El mes ${yearMonth} ya se encuentra cerrado.`
    );
  }
