...
export const DoctorPatientHeader: React.FC<DoctorPatientHeaderProps> = ({
  ...
  // Sort consultations to get the latest control
  const sortedConsultations = React.useMemo(() => {
    return [...selectedPatientConsultations].sort(
      (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
    );
  }, [selectedPatientConsultations]);

  const lastControl = sortedConsultations[0];
  const activeDiagnoses = selectedPatient.medicalHistory || [];
  const allergies = selectedPatient.allergies || [];
  const drugAllergies = allergies.filter((a) => a.type === "Farmaco");
  const otherAllergies = allergies.filter((a) => a.type !== "Farmaco");

  return (
    <header className="bg-white border-b border-slate-100 px-8 py-4 flex flex-col gap-3 sticky top-0 z-30 shadow-sm backdrop-blur-md bg-white/90 pt-20 lg:pt-16">
              ...
              <>
                <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2 group leading-none">
                  ...
                  <span className="px-4 py-1 bg-indigo-50 text-indigo-700 text-xl rounded-full font-mono font-black border-2 border-indigo-100 shadow-md tracking-tighter">
                    {isPiiMasked ? maskRUT(selectedPatient.rut) : selectedPatient.rut}
                  </span>
                  {!isReadOnly && (
                    <button
      ...
      {/* Clinical Decision Header Strip */}
      {!isEditingPatient && (
        <div className="flex flex-wrap items-center gap-4 py-2.5 px-4 bg-slate-50/80 rounded-xl border border-slate-200/60 text-xs text-slate-600 shadow-inner w-full font-medium">
            ...
            <div className="flex flex-wrap gap-1">
              {activeDiagnoses.length > 0 ? (
                activeDiagnoses.map((diag, idx) => {
                  const label = typeof diag === "string" ? diag : diag.display;
                  return (
                    <span
                      key={idx}
                      className="bg-blue-50 text-blue-700 font-semibold px-2 py-0.5 rounded-lg border border-blue-100"
                    >
                      {label}
                    </span>
                  );
              ...
              {allergies.length > 0 ? (
                <>
                  {drugAllergies.map((a, idx) => (
                    <span
                      key={`drug-${idx}`}
                      className="bg-red-50 text-red-750 font-bold px-2 py-0.5 rounded-lg border border-red-150 shadow-sm animate-pulse flex items-center gap-1"
                      title={`Reacción: ${a.reaction}`}
                    >
                      ⚠️ FÁRMACO: {a.substance}
                    </span>
                  ))}
                  {otherAllergies.map((a, idx) => (
                    <span
                      key={`other-${idx}`}
                      className="bg-amber-50 text-amber-700 font-semibold px-2 py-0.5 rounded-lg border border-amber-100 shadow-sm"
                      title={`Reacción: ${a.reaction}`}
                    >
                      {a.substance}
                    </span>
                ...
                <span className="bg-emerald-50 text-emerald-700 font-semibold px-2 py-0.5 rounded-lg border border-emerald-100">
          ...
          <div className="flex items-center gap-1.5 ml-auto text-slate-500">
            ...
            {lastControl ? (
              ...
              <span className="text-slate-400 italic">Primer control en Clave Salud</span>
            )}
          </div>
        </div>
      )}
