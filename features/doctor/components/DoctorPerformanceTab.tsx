...
import { collection, doc, onSnapshot, getDoc, query, where, orderBy, getDocs } from "firebase/firestore";
import { Appointment, MonthlyProfessionalStats } from "../../../types";
import { Activity, Download, FileText, Search, TrendingUp, DollarSign, XCircle, CheckCircle } from "lucide-react";
...
export const DoctorPerformanceTab: React.FC<DoctorPerformanceTabProps> = ({
    ...
    // Export CSV
    const exportCSV = () => {
        const headers = ["Fecha", "Hora", "Paciente", "RUT", "Servicio/Tipo", "Estado Agendamiento", "Asistencia", "Cobrado", "Monto"];
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                <div>
                    ...
                    <p className="text-slate-500">Consulta tus estadísticas mensuales y exporta el detalle de atenciones.</p>
                </div>
            ...
            {loadingStats ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex flex-col justify-center relative overflow-hidden">
                        ...
                        <p className="text-4xl font-extrabold text-slate-800 z-10">{stats?.totalAppointments || 0}</p>
                    </div>

                    <div data-testid="doctor-kpi-completed" className="bg-emerald-50 rounded-2xl p-6 border border-emerald-100 shadow-sm flex flex-col justify-center relative overflow-hidden">
                    ...
                    <div className="bg-rose-50 rounded-2xl p-6 border border-rose-100 shadow-sm flex flex-col justify-center relative overflow-hidden">
                        ...
                        <p className="text-4xl font-extrabold text-rose-700 z-10">{(stats?.cancelled || 0) + (stats?.noShow || 0)}</p>
                    </div>

                    <div data-testid="doctor-kpi-revenue" className="bg-indigo-50 rounded-2xl p-6 border border-indigo-100 shadow-sm flex flex-col justify-center relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-4 opacity-10">
                        ...
                        <span className="text-xs text-indigo-500/80 font-medium z-10 mt-1">({stats?.billableCount || 0} atenciones)</span>
                    </div>
                </div>
            )}
                            ...
                            {loadingAppointments ? (
                                ...
                                appointments.map((a) => (
                                    <tr key={a.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td className="p-4">
                                            <span className={`px-3 py-1 rounded-full text-xs font-bold ${a.status === "booked" ? "bg-amber-100 text-amber-700" : "bg-green-100 text-green-700"
                                                }`}>
                                        ...
                                        <td className="p-4">
                                            {a.attendanceStatus === "completed" ? (
                                                <span className="flex items-center gap-1 text-emerald-600 text-sm font-bold"><CheckCircle className="w-4 h-4" /> Atendido</span>
                                            ) : a.attendanceStatus === "no-show" ? (
                                                <span className="flex items-center gap-1 text-rose-600 text-sm font-bold"><XCircle className="w-4 h-4" /> No Asistió</span>
                                            ) : a.attendanceStatus === "cancelled" ? (
                                                <span className="flex items-center gap-1 text-slate-400 text-sm font-bold">Anulado</span>
                                            ) : (
                                                <span className="text-slate-400 text-sm">- Pendiente -</span>
                                            )}
                                        </td>
                                        <td className="p-4 text-right">
                                            <div className={`font-bold ${a.billable ? "text-indigo-600" : "text-slate-400"}`}>
