...
export const AdminPerformanceTab: React.FC<AdminPerformanceTabProps> = ({
  ...
  useEffect(() => {
    ...
    const q = query(profStatsRef, where("yearMonth", "==", yearMonth));
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div>
          ...
          <p className="text-slate-500">
            Supervisa el rendimiento del centro, genera reportes de profesionales y realiza cierres
            mensuales.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          {isMonthClosed ? (
            <div
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 text-slate-600 rounded-full text-sm font-bold border border-slate-200"
            >
            ...
            <div
              data-testid="month-status-badge"
              className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-full text-sm font-bold border border-emerald-200"
            >
      ...
      {loadingStats ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div
            data-testid="kpi-total-appointments"
            className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex flex-col justify-center relative overflow-hidden"
          >
            ...
            <p className="text-4xl font-extrabold text-slate-800 z-10">
              {centerStats?.totalAppointments || 0}{" "}
              <span className="text-lg text-slate-500">citas</span>
            </p>
          </div>
          ...
          <div className="bg-emerald-50 rounded-2xl p-6 border border-emerald-100 shadow-sm flex flex-col justify-center relative overflow-hidden">
            ...
            <p className="text-4xl font-extrabold text-emerald-700 z-10">
              {centerStats?.completed || 0}
            </p>
          </div>
          ...
          <div className="bg-rose-50 rounded-2xl p-6 border border-rose-100 shadow-sm flex flex-col justify-center relative overflow-hidden">
            ...
            <p className="text-4xl font-extrabold text-rose-700 z-10">
              {(centerStats?.cancelled || 0) + (centerStats?.noShow || 0)}
            </p>
          </div>

          <div
            data-testid="kpi-total-revenue"
            className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl p-6 shadow-md flex flex-col justify-center relative overflow-hidden text-white"
          >
          ...
          <table data-testid="prof-stats-table" className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 text-slate-500 text-sm border-b border-slate-100">
                ...
                <th className="p-4 font-bold text-center" title="Completadas">
                  Efectivas
                </th>
                <th className="p-4 font-bold text-center" title="No-Show o Anuladas">
                  Ausencias
                </th>
                <th className="p-4 font-bold text-center" title="Citas Facturables (Completadas)">
                  Facturables
                </th>
              ...
              {profStats.length === 0 && !loadingStats ? (
                ...
                profStats.map((p) => {
                  ...
                  return (
                    ...
                    >
                      <td
                        data-testid={`prof-name-${p.doctorId}`}
                        className="p-4 font-bold text-slate-800 flex items-center gap-3"
                      >
