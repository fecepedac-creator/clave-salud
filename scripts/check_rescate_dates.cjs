    ...
    for (const c of centersSnap.docs) {
        ...
        for (const p of patientsSnap.docs) {
            ...
            if (lastConsult.nextControlDate) {
                console.log(`[Revisado hoy] Paciente ${data.fullName} -> Próximo control exacto anotado: "${lastConsult.nextControlDate}"`);
            } else {
                console.log(`[Paciente ${data.fullName}] Última consulta sin nextControlDate. Objeto: ${JSON.stringify(lastConsult).slice(0, 150)}...`);
            }
        }
    }
}
