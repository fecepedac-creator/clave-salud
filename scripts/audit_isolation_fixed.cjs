    ...
    if (coll.isGlobal) {
      ...
      snapshot.forEach((doc) => {
        ...
        const hasCenterId =
          data.centerId ||
          (data.accessControl &&
            data.accessControl.centerIds &&
            data.accessControl.centerIds.length > 0);

        if (!hasCenterId) {
          console.warn(
            `[REGISTRO HUÉRFANO DETECTADO] ID: ${doc.id} - Nombre: ${data.fullName || "N/A"}`
          );
          orphans++;
        } else {
          healthy++;
        ...
        for (const sub of subCollections) {
          ...
          subSnap.forEach((doc) => {
            ...
            if (data.centerId && data.centerId !== centerId) {
              console.error(
                `[CRITICAL] Data Mismatch! Registro ${doc.id} en centro ${centerId} tiene centerId ${data.centerId}`
              );
              problematic++;
            }
          });
          if (problematic > 0) {
            console.error(
              `Centro: ${centerName} (${centerId}) - ${sub}: ${problematic} errores de mismatch.`
            );
          }
        }
      }
