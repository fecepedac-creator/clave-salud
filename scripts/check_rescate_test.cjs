...
async function check() {
    ...
    for (const c of centersSnap.docs) {
        const centerId = c.id;
        const patientsSnap = await db.collection("centers").doc(centerId).collection("patients").where("active", "==", true).get();
        for (const p of patientsSnap.docs) {
            ...
            if (lastConsult.nextControlDate === targetDateStr) {
                ...
                console.log(`- ¿Cita agendada a futuro con este Dr?: ${hasFutureApp ? 'SÍ (Será ignorado por el bot)' : 'NO (Recibirá WhatsApp de Autorescate)'}`);

                const hasExams = (lastConsult.prescriptions || []).some(p => p.type === "OrdenExamenes" || p.type === "Solicitud de Examen");
                console.log(`- ¿Tiene órdenes de exámenes pendientes?: ${hasExams ? 'SÍ (Bot detectará y adaptará template)' : 'NO (Flujo normal)'}`);
    ...
    if (countTotal === 0) {
        console.log("No se encontró ningún paciente que cumpla la condición. ¿Seguro que guardaste la Ficha Clínica y en el campo Final 'Próximo control' le marcaste '13 de marzo del 2026'?");
    }
}
