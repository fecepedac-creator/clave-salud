    ...
    for (const c of centersSnap.docs) {
        ...
        for (const p of patientsSnap.docs) {
            ...
            if (lastConsult.nextControlDate === targetDateStr) {
                console.log(`[Paciente Encontrado] Felipe, el paciente ${data.fullName} SÍ califica! Fecha Control: ${lastConsult.nextControlDate}`);
                ...
                console.log(`  - Tiene cita agendada a futuro con este Dr?: ${hasFutureApp ? 'SÍ (Será ignorado)' : 'NO (Recibirá WhatsApp)'}`);
