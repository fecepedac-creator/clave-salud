...
async function inspect() {
  ...
  const userSnapshot = await db
    .collection("users")
    .where("emailLower", "==", targetEmail.toLowerCase())
    .get();
  ...
  staffSnapshot.forEach((doc) => {
    const data = doc.data();
    log(
      `Staff ID: ${doc.id} | Name: ${data.fullName || data.nombre} | Email: ${data.email || data.emailLower} | Role: ${data.role}`
    );
  ...
  inviteSnapshot.forEach((doc) => {
    log(
      `Invite Token: ${doc.id} | Status: ${doc.get("status")} | Role: ${doc.get("role")} | Center: ${doc.get("centerName")}`
    );
    log(JSON.stringify(doc.data(), null, 2));
  });
